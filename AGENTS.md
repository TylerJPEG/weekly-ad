...
- Mobile allows native one-finger panning on both axes. At `Fit`, the lack of horizontal overflow makes this feel like ordinary vertical page scrolling; when zoomed in, the same gesture pans the two-dimensional canvas. Two-finger gestures change flyer zoom rather than being required for panning.
...
- While the item list is open, a stationary click or tap on blank gray flyer canvas closes it and preserves the active page. Flyer-page clicks and floating controls keep their existing actions; dragging, scrolling, and pinching must not trigger canvas dismissal.
- Treat flyer pages as one keyboard-navigation component. Only one page is in the Tab sequence at a time; arrow keys move focus and active-page state within the collection, Home and End move to its boundaries, and Enter or Space uses the focused page's list action. Tabbing again leaves the page collection. Do not capture page-navigation arrow keys while focus is on unrelated controls.
...
- Do not display a visible numeric zoom percentage. Label the fitted-state action `Fit`, give it a more descriptive accessible name, disable it at `Fit` (100%), and enable it whenever the flyer zoom differs from `Fit`.
...
- Focus-visible outlines on buttons inside clipped control groups, including the minus, `Fit`, and plus zoom group, must use a negative outline offset so the complete focus indicator remains visible inside the group.
